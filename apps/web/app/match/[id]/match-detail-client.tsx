            ...
            <Card className="overflow-hidden rounded-2xl border-border shadow-sm p-0">
                ...
                <div className="p-6 md:p-10">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16">
                        <div className="flex flex-col items-center gap-4 w-32 md:w-48">
                            <Flag team={match.home} size="lg" className="size-20 md:size-24 rounded-md" />
                            ...
                            <div className="mt-4">
                                {isLive ? (
                                    <Badge className="bg-accent text-accent-foreground px-4 py-1.5 font-mono text-sm font-bold uppercase tracking-wider">
                                        ...
                                        {t('match.live', { m: match.minute ?? 0 })}
                                    </Badge>
                                ) : match.status === 'finished' ? (
                                    <Badge variant="secondary" className="px-4 py-1.5 font-mono text-sm font-bold uppercase tracking-wider text-muted-foreground">
                                        {t('match.ft')}
                                    </Badge>
                                ) : (
                                    <Badge variant="outline" className="px-4 py-1.5 font-mono text-sm font-bold uppercase tracking-wider text-muted-foreground">
                                        {t('match.upcoming')}
                                    </Badge>
                                )}
                            </div>
                        ...
                        <div className="flex flex-col items-center gap-4 w-32 md:w-48">
                            <Flag team={match.away} size="lg" className="size-20 md:size-24 rounded-md" />
                            <span className="text-xl md:text-2xl font-bold text-center">
                ...
                <div className="bg-secondary/30 border-t border-border p-6 md:p-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center md:text-left">
                        ...
                        <div>
                            ...
                            <p className="font-medium">{cityName(match.city, lang)}</p>
                        </div>
                    </div>
                </div>
            </Card>
